...
import static service.businesslogic.exception.TalkValidationException.ADDITIONAL_COMMENT_TOO_LONG;
import static service.businesslogic.exception.TalkValidationException.NOT_ALLOWED_TO_UPDATE;
import static service.businesslogic.exception.TalkValidationException.ORG_COMMENT_IS_EMPTY;
import static service.businesslogic.exception.TalkValidationException.ORG_COMMENT_TOO_LONG;
...
import service.infrastructure.mail.preparator.ChangeTalkStatusOrganiserPreparator;
import service.infrastructure.mail.preparator.ChangeTalkStatusSpeakerPreparator;
...
public class TalkServiceImpl implements TalkService {
	...
	public void updateAsOrganiser(TalkDto talkDto, User user) {
		...
		talkRepository.save(talk);
		List<User> receivers = userRepository.findAllByRolesIsIn(roleRepository.findByName(Role.ORGANISER)).stream()
				.filter(u -> u != user).collect(Collectors.toList());
	...
	@Override
	public List<Submission> getTalksForSpeaker(String userEmail) {
	...
	private void checkDtoBeforeUpdateAsOrganiser(TalkDto talkDto) {
		...
		} else if (talkDto.getStatusName().equals(TalkStatus.REJECTED.getName())
				&& (talkDto.getOrganiserComment() == null || talkDto.getOrganiserComment().isEmpty())) {
